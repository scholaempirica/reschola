...
schola_project <- function(path, ...) {
  ...
  switch(dots[["vcs"]],
    ...
    "Schola's private GitHub repo" = {
      tryCatch(use_git(message = "Repo setup"), error = function(cnd) {
        ui_oops("Local git initialization failed. Do you have git installed?")
      })
      tryCatch(
        use_github(organisation = "scholaempirica", private = TRUE),
        error = function(cnd) {
          ui_oops(
            "Linking GitHub with your project failed. You may have to authenticate yourself. See {ui_code('?gh::gh_token')}."
          )
        }
      )
    }
  ...
  if (!is.null(dots$drive_folder) && dots$drive_download) {
    gdrive_dribble <- tryCatch(
      as_dribble(dots$drive_folder),
      error = function(cnd) {
    ...
    if (!is_folder(gdrive_dribble)) {
      ui_stop(
        "It seems the Google Drive folder URL you have given does not point to a folder."
      )
    }
  ...
  if (dots$drive_download && is.null(dots$drive_folder)) {
    ui_oops(
      "You asked for the GDrive files to be downloaded, but provided no URL.\nNothing was downloaded."
    )
  }
