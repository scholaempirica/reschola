...
schola_project <- function(path, ...) {
  ...
  if(dots$drive_folder != "") {
    tryCatch(gdrive_dribble <- as_dribble(dots$drive_folder),
             error = function(cnd) ui_oops("You need to authenticate R to your Google Drive account.
  ...
  switch(dots[["vcs"]],
    ...
    "GitHub: public owned by SCHOLA EMPIRICA" = {
      use_git()
      use_github(organisation = "scholaempirica")
    },
  )
  ...
  readme_text <- c(str_glue("# {title_readme}\n\n\n"),
  "<!-- badges: start -->",
  "<!-- badges: end -->\n",
  ...
  file_copy(gs_repro, "99_reproducibility.Rmd")

  gs_bookdown <- reschola_file(
    "rstudio", "templates", "project",
    "proj_fls", "_bookdown.yml"
  )
  file_copy(gs_bookdown, "_bookdown.yml")
  ...
  use_git_ignore("*", "data-input") # have git ignore everything in that dir
  use_git_ignore("!.gitignore", "data-input") # except the .gitignore, so the dir will be committed
  dir_create("data-processed")
  use_git_ignore("*", "data-processed") # ditto
  use_git_ignore("!.gitignore", "data-processed") #ditto
  dir_create("data-output")
  dir_create("charts-output")
  dir_create("reports-output")
  ...
  file_copy(gs_retrieve_script, "001_retrieve-data.R")

  # print("Making dummy files")

  file_create("002_read-data.R")
  file_create("003_check-and-process-data.R")
