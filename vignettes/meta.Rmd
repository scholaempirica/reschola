...
This package is being developed on Github using the workflow suggested Hadley Wickham's R Packages book and implemented by the [`usethis`](https://github.com/r-lib/usethis) package.
...
An exception to the automatic continuous integration and check pipeline is the `chart.Rmd` vignette, which is prebuilt locally using `vignettes/prebuild.r` from `charts.Rmd.orig` to ensure fonts are used correctly.

#### Test coverage

Currently no tests are included.
...
This is done manually, using the `ptrr::insert_package_into_drat()` utility in the ptrr package but can be done completely manually manually by downloading built binaries output by r-hub and inserting it into the `gh-pages` branch of the `drat` Github repository.
